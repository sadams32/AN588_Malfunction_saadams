...
### Point Estimate of Slope & Hypotheses Testing
```{r}
# Model 1
lm_beta1 <- cor(d$MaxLongevity_m, d$Brain_Size_Species_Mean) * sd(d$MaxLongevity_m) / sd(d$Brain_Size_Species_Mean)
lm_beta1

lm_beta0 <- mean(d$MaxLongevity_m) - lm_beta1 * mean(d$Brain_Size_Species_Mean)
lm_beta0

# Model 2
log_lm_beta1 <- cor(d$log_longevity, d$log_brainsize) * sd(d$log_longevity) / sd(d$log_brainsize)
log_lm_beta1

log_lm_beta0 <- mean(d$log_longevity) - log_lm_beta1 * mean(d$log_brainsize)
log_lm_beta0
```
Conclusion: 𝛽1 ≠ 0 for either model, so we reject the null hypothesis in favor of the alternative (there is a statistically significant correlation between life span and brain size for this sample).

### 90% Confidence Intervals
```{r}
# Model 1
CI1 <- confint(lm_model, level = 0.90)
CI1

# Model 2
CI2 <- confint(log_lm_model, level = 0.90)
CI2
...
ggplot(d, aes(x = MaxLongevity_m, y = Brain_Size_Species_Mean)) + geom_point() + geom_text(x = max(d$MaxLongevity_m), y = max(d$Brain_Size_Species_Mean, label = lm_formula, color = "blue")) + labs(title = "Longevity (mo.) vs. Brain Size (g)", x = "Longevity (months)", y = "Brain Size (grams)")
```
