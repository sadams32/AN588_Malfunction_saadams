...
Z.prop.test <- function(p1, n1, p2 = NULL, n2 = NULL, p0, alternative = "two.sided", conf.level = 0.95) {
  ...
  check.norm <- function(p,n){
    if (n * p <= 5 && n * (1 - p) <= 5){
      ...
      # CM Commentary: Nice addition of the warning message!
    }
  }
  check.norm(p1, n1) # CM Commentary: Would you be able to elaborate on what check.norm() does?
...
`CM: I used else! But you make a good point of giving a warning message if an alternative value is provided!`

`CM: Good function! We definitely have very similar ones but that's because we all based it off from the functions shown in Module 11! In my code I included an example to test if the function worked as expected! I used the example from the module but I think it would be a good idea to test any values against the prop.test() function!`
...
d <- na.omit(d)
# CM: This is a nice detail! I didn't do this but was surprised that the code was able to run anyway!
...
`CM: This is an absolute nitpick, but I think rounding the numbers to two/three decimal places will make the function look neater! You can do this with the round() function!`
...
`CM: I think including the argument formula = y ~ x in geom_smooth() might be the thing you're looking for! I like the idea of writing a function to suit both models but I think that the hard part would be accomodating the coordinates where you'll display the formula!`
...
# CM: Interesting! I tried doing this with normal indexing and I think it works! See below!
...
`CM: See comment I left in code chunk above!`

Interpretation: reject the null hypothesis in favor of the alternative. That is, there is a significant linear relationship between brain size and longevity in both models (because beta1 does not equal 0 and the 0 is not contained in the confidence intervals for either model.) `CM: Nice explanation!`
...
`CM: What matters is that it works! And It looks super neat!`
...
//Challenge// I don't know if I should expand on my interpretation or if there is more to be said for how to look at the PIs and CIs. `CM: I like this explanation! I think it is way better explained than mine lol!`
